...
(lambda l,r=__import__("random"),f=lambda t,o=9,p="%02d:%02d\t":p%(o+t//60,t%60),p=print,t=540:r.seed(int(l(lambda _:input("Entrez une seed: "),str.isdigit)))or(p(f(l(lambda b,d=r.randint:((b[0]+20,b[1]-20,b[2],p(f(b[0])+"C'est bon, encore le temps de faire une sieste. Zzz"))if 20<=b[1]<=50else(min(b[0]+b[1]+90,t),-1,b[2]+(min(b[0]+b[1]+90,t)-b[0]),p(f(b[0])+"Il faut travailler. M'enfin."+(("\n"+f(b[0]+b[1]+90)+"Prunelle est parti. \\O/")if t>b[0]+90+b[1]else""))))if-1<b[1]<50else(p(f(b[0])+"OK, pause !")or b[0]+50,max(-1,b[1]-50)if-1!=b[1]or d(0,2)else(lambda a,t=b[0]+50:p(f(t)+"Attention, Prunelle arrive à "+f(t+a))or a)(d(10,60)),b[2]),lambda b:b[0]>=t,(0,-1,0,p(f(0)+"Gaston arrive au bureau.")))[2],0,f(t)+"Fin du service, dure journée\nTemps total travaillé : %d h %02d min"))))(lambda a,c,i=0,s=lambda a,c,x,s:x if c(x)else s(a,c,a(x),s):s(a,c,a(i),s))
